                    ...
                    if (lastTrigerPrice > 0) //判断是否触发移动止盈
                    {
                        switch (pos.SideType)
                        {
                            case PositionType.Long: //多头持仓的情况                                
                                ...
                                {
                                    operationDes = $"多头回撤:{RetracementPercent}%后止盈 剩余盈利:{profit.ToString("F2")}% 标准盈利:{StopSurplus}% 开仓均价:{pos.AvgPx.ToString("F2")} 回撤价:{longStopPrice.ToString("F2")} 最高价:{lastTrigerPrice} 平仓价:{closePrice.ToString("F2")}";
                            ...
                            case PositionType.Short: //空头持仓的情况
                                ...
                                {
                                    operationDes = $"空头回撤:{RetracementPercent}%后止盈 剩余盈利:{profit.ToString("F2")}% 标准盈利:{StopSurplus}% 最低价:{lastTrigerPrice} 回撤价:{shortStopPrice.ToString("F2")} 开仓均价:{pos.AvgPx.ToString("F2")} 平仓价:{closePrice.ToString("F2")}";
                                    operationProfit = pos.Margin * profit / 100;
                                    Logger.Instance.LogInfo(operationDes);
                                }
                                else if (profit < (decimal)StopSurplus) // 不吃亏，既有利润不能被侵犯
